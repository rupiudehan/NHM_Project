    ...
    function Insert() {
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                ...
                StockCertificateDocument: StockCertificateDocument
            }),
            contentType: "application/json;",
            dataType: "json",
            asyn: false,
...
function Edit(MillerPerformaId) {
    ...
    $.ajax({
        ...
        success: function (result) {
			...
			EditBindMiller(strdata[0].DistrictId, 38, strdata[0].MillerId);

            //$('#ddlCropYear').val(36);
			//$('#ddlCropYear').val(38);
			            cropyear = strdata[0].strCropYear;
			$('#ddlCropYear').val(38);
            $("#ddlCropYear").trigger("liszt:updated");
    ...
    $('#ddlCentre option[value=' + CenterID + ']').attr("selected", true);
	    $('#ddlCropYear option[value=' + cropyear + ']').attr("selected", true);
}
